    ...
    <vx-card class="grid-view-item mb-base overflow-hidden" v-on="$listeners">
        <template slot="no-body">
            ...
            <div class="item-img-container bg-white h-64 flex items-center justify-center mb-4 cursor-pointer" @click="navigate_to_detail_view">
                <img :src="'images/' + item.image" :alt="item.name" class="grid-view-img px-4">
            </div>
            ...
            <!-- SLOT: ACTION BUTTONS -->
            <slot name="action-buttons" :product="item"/>
        </template>
    </vx-card>
    <!--  </div>
 </div> -->
...
export default{
    ...
    methods: {
        navigate_to_detail_view() {
            this.$router.push({name: 'product', params: {item_id: this.item.id }})
                .catch(() => {})
        }
    }
}
